...
function compute_loss_q(
	...
	q_ensemble = ac.qs[iter]
	o, a, r, o2, d = data
	r = getindex.(r, iter)
	qs = [q(o, a) for q in q_ensemble]
	a2, logp_a2 = ac.pi(o2)

	# Target q-values
	q_targ_ensemble = ac_targ.qs[iter]
	qs_pi_targ = [q(o2, a2) for q in q_targ_ensemble]
...
function compute_loss_pi(ac::MLPActorCritic, data::NamedTuple, alpha::Vector{Float64})
    o = data.obs
    pi, logp_pi = ac.pi(o)
    qs_pi = [[q(o, pi) for q in outer_qs] for outer_qs in ac.qs]
	q_pi = [sum(qs_pi_vec) / length(qs_pi_vec) for qs_pi_vec in qs_pi] #min.(qs_pi...)
...
function update(
	...
	data = (; zip(keys(data), cpu.(values(data)))...)
    # Gradient descent step for value networks
	loss_q = 0.0
	q_ps = params((q -> q.q).(reduce(vcat,ac.qs)))
	q_gs = gradient(q_ps) do
		for i in 1:3
			iter = i
	        loss_q = loss_q + compute_loss_q(ac, ac_targ, data, gamma, alpha,iter)
		end
		return loss_q
	end
	...
	params((q -> q.q).(reduce(vcat,ac.qs)))
    # Update target networks with in-place Polyak averaging
	for (dest, src) in zip(params((q -> q.q).(reduce(vcat,ac_targ.qs))), params((q -> q.q).(reduce(vcat,ac.qs))))
...
function test_agent(
    ...
    for _ in 1:num_test_episodes
        ...
        while !(d || ep_len == max_ep_len)
            a = ac(o, true)
            qs = [[q.q(vcat(o, a))[] for q in outer_qs] for outer_qs in ac.qs]
            r = act!(test_env, a)
            o = observe(test_env)
            d = terminated(test_env)
            ep_ret += sum(r)
            ep_len += 1
			qs_stdev = [std(q) for q in qs]
            push!(stdevs, sum(qs_stdev))
        end
		# if num_test_episodes & 25 == 0
		# 	println(ep_ret)
		# end
...
Base.@kwdef mutable struct SAC
    ...
    gamma::Float64 = 1.00	                        # discount factor was 0.95
	...
	num_q_outer::Int = 3							# number of ensembles needed for the new semantic reward function
...
function solve(sac::SAC)
    ...
    ac = MLPActorCritic(sac.obs_dim, sac.act_dim, sac.act_mins, sac.act_maxs,
        sac.hidden_sizes, sac.num_q, sac.num_q_outer, sac.activation, sac.rng)
    ac_targ = deepcopy(ac)
    alpha = [1.0]
    ...
    for t in 1:total_steps
		...
		a = random_policy ? rand(actions(env)) : ac(cpu(o)) |> cpu
        # Step environment
        r = act!(env, a)
        o2 = observe(env)
        d = terminated(env)
