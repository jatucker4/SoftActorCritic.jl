...
function SquashedGaussianMLPActor(
	...
	net = mlp(vcat(obs_dim, hidden_sizes), activation, activation) |> cpu
    mu_layer = Dense(hidden_sizes[end], act_dim) |> cpu
    log_std_layer = Dense(hidden_sizes[end], act_dim) |> cpu
    act_mins, act_maxs = cpu(Float32.(act_mins)), cpu(Float32.(act_maxs))
    return SquashedGaussianMLPActor(net, mu_layer, log_std_layer, act_mins, act_maxs, rng)
end
...
function (pi::SquashedGaussianMLPActor)(
    ...
    if deterministic
		...
		deviate = mu isa CuArray ? cpu(deviate) : deviate
...
function MLPActorCritic(
    ...
    qs = [[MLPQFunction(obs_dim, act_dim, hidden_sizes, activation) for _ in 1:num_q] for _ in 1:num_q_outer]
